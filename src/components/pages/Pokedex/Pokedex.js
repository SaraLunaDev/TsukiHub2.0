...
function Pokedex() {
  ...
  const activeGeneration =
    REGIONS.find((r) => r.name.toLowerCase() === activeRegion)?.generation || 1;
    ...
    parsedData.forEach((pokemon) => {
      if (pokemon.Usuario) {
        const userData = usersData?.find(
          (u) => String(u.id).trim() === String(pokemon.Usuario).trim(),
        );
        if (!userMap.has(pokemon.Usuario)) {
          userMap.set(pokemon.Usuario, {
            id: pokemon.Usuario,
            nombre: userData?.nombre || `Usuario ${pokemon.Usuario}`,
            pfp:
              userData?.pfp ||
  ...
  useEffect(() => {
    if (userSearchText && filteredUsers.length === 1) {
      setSelectedUser(filteredUsers[0].id);
    } else if (userSearchText && filteredUsers.length > 1) {
      const exactMatch = filteredUsers.find(
        (user) => user.nombre.toLowerCase() === userSearchText.toLowerCase(),
      );
      if (exactMatch) {
        setSelectedUser(exactMatch.id);
      }
    }
  }, [userSearchText, filteredUsers, setSelectedUser]);
  ...
  const pokemonGrid = useMemo(() => {
    ...
    return Array.from({ length: generationSize }, (_, index) => {
      ...
      const matchedPokemon = pokemonList.find(
        (p) => parseInt(p.pokemonId, 10) === id && p.Usuario === selectedUser,
      );
  ...
  const { totalCaptured, completionPercentage } = useMemo(() => {
    ...
    const percentage =
      totalPossible > 0 ? ((captured / totalPossible) * 100).toFixed(0) : 0;
                  ...
                  return (
                    <button
                      ...
                      className={`pokedex-region-button ${
                        activeRegion === reg.name.toLowerCase() ? "active" : ""
                      }`}
                      onClick={() => handleRegionChange(reg.name.toLowerCase())}
                    >
                      <IconComponent className="nav-icon" />
                      <span className="nav-text">{reg.name}</span>
                    </button>
                  );
                })}
              </div>
