...
function Gacha() {
        ...
        if (userId && userName) {
          ...
          BANNERS.forEach((b) => {
            for (let tier = 3; tier <= 5; tier++) {
              ...
              if (tierChars) {
                const chars = tierChars.split("/-/").filter((c) => c.trim());
  ...
  const { tier3Grid, tier4Grid, tier5Grid } = useMemo(() => {
    ...
    for (let t = 3; t <= 5; t++) {
      ...
      ownedSets[t] = new Set(
        tierCharsStr
          ? tierCharsStr.split("/-/").map((c) => c.trim().toLowerCase())
          : [],
      );
    }

    characters.forEach((char) => {
      const owned = ownedSets[char.tier]?.has(char.name.toLowerCase()) || false;
  ...
  }, [characters, selectedUser, userData, activeBanner, totalCharactersGlobal]);
