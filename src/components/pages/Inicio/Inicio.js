...
function Inicio() {
	...
	const achievementDetails = {
		l_racha: {
			...
			description:
				"Escribir un mensaje durante 100 directos consecutivos",
			color: "#FFD700",
		},
		l_mensajes: {
		...
		l_pokedex: {
			name: "Apice de Kanto",
			description:
				"Consigue capturar todos los Pokemons de la region de Kanto",
			color: "#C0C0C0",
		},
		l_eevees: {
		...
		l_gacha: {
			name: "Mano Divina",
			description:
				"Consigue cinco o mas cartas 5★, sin importar el banner",
			color: "#cd7f32",
		},
	};
	...
	useEffect(() => {
		...
		const updateMaxEmotes = () => {
			...
			const hasChanges =
				...
				Object.keys(maxEmotesPerRow).length !==
		...
		return userData
			...
			.sort((a, b) => {
				if (field === "racha") {
					...
					const aEmotes = Array.isArray(a[field])
						? a[field].length
						: 0;
					const bEmotes = Array.isArray(b[field])
						? b[field].length
						: 0;
				...
				<StatSection
					...
					subtitle={`${
						userData.filter((user) => {
							...
							const rachaNum =
								rachaStr.startsWith("m_") ||
								rachaStr.startsWith("f_")
						...
						{showEmotesTable ? (
							<StatTable
								...
								rows={getTopUsers(
									...
									const startIndex =
										currentLineIndex * maxEmotesPerLine;
								...
								columns={[
									...
									},
									{
										...
										render: (row) => {
											...
											const isFirstLine =
												row.currentLineIndex === 0;
											const isLastLine =
												row.currentLineIndex ===
												row.totalLinesNeeded - 1;
											const isMiddleLine =
