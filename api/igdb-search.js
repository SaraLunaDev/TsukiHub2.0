  ...
  try {
    ...
    if (!tokenData.access_token) {
      ...
      const igdbResp = await fetch("https://api.igdb.com/v4/games", {
        ...
        body: `search "${query}"; fields id,name,cover.url,first_release_date,genres.name,platforms.name,summary,involved_companies.company.name,videos.*,total_rating,artworks; limit 10;`,
      });
    ...
    const resultsWithTrailer = igdbData.map((game) => {
      ...
      const results = resultsWithTrailer.map((game) => {
        ...
        let artworkUrl = "";
        if (game.artworks && Array.isArray(game.artworks) && game.artworks.length > 0) {
