export default async function handler(req, res) {
  ...
  try {
    ...
    const igdbResp = await fetch("https://api.igdb.com/v4/games", {
      ...
      body: `search "${query}"; fields id,name,cover.url,first_release_date,genres.name,platforms.name,summary,involved_companies.company.name,videos.*,total_rating; limit 10;`,
    });
    ...
    const results = resultsWithTrailer.map((game) => ({
      ...
      caratula: game.cover
        ...
        : "",
      imagen: game.cover
        ? game.cover.url.replace("t_thumb", "t_screenshot_big")
        : "",
      duracion: "",
      nota: "",
      trailer: (() => {
